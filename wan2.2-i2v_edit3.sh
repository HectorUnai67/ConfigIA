...
main() {
    ...
    download_input
    download_gdrive_loras "$GDRIVE_LORAS_FOLDER_ID" "$MODELS_DIR/loras"
...
# Uso: download_gdrive_loras <folder_id> <dest_dir>
download_gdrive_loras() {
  ...
  # Listar todos los elementos de la carpeta (paginado)
  local page_token=""
  while true; do
    local url="${api}/files?q='${folder_id}'+in+parents+and+trashed=false&fields=nextPageToken,files(id,name,mimeType)&key=${GDRIVE_API_KEY}&pageSize=1000"
    ...
    # Procesar cada elemento de la carpeta
    local ids names mimeTypes
    ids=$(echo "$response" | python3 -c "import sys,json; items=json.load(sys.stdin)['files']; [print(i['id']) for i in items]")
    names=$(echo "$response" | python3 -c "import sys,json; items=json.load(sys.stdin)['files']; [print(i['name']) for i in items]")
    mimeTypes=$(echo "$response" | python3 -c "import sys,json; items=json.load(sys.stdin)['files']; [print(i['mimeType']) for i in items]")

    # Iterar en paralelo sobre ids, names y mimeTypes
    while IFS= read -r id && IFS= read -r name <&3 && IFS= read -r mime <&4; do
      if [ "$mime" = "application/vnd.google-apps.folder" ]; then
        echo "==> Carpeta: $dest_dir/$name"
        download_gdrive_loras "$id" "$dest_dir/$name"
      else
        local dest_file="$dest_dir/$name"
        if [ -f "$dest_file" ]; then
          echo "  Ya existe, saltando: $name"
        else
          echo "  Descargando: $name -> $dest_file"
          curl -L --progress-bar \
            "${api}/files/${id}?alt=media&key=${GDRIVE_API_KEY}" \
            -o "$dest_file" \
            || echo "[WARN] Error descargando $name"
        fi
      fi
    done < <(echo "$ids") 3< <(echo "$names") 4< <(echo "$mimeTypes")

    # Siguiente página si existe
    page_token=$(echo "$response" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('nextPageToken',''))" 2>/dev/null || true)
    [ -z "$page_token" ] && break
  done

  echo "✓ Carpeta de Drive descargada en: $dest_dir"
}
